...
pro m3_photom_full_correction_gui_start_with_thermrem, event
;
...
tlb=widget_base(row=5, xoffset=xoff, yoffset=yoff, tab_mode=1, title='M3 Photometric Correction')
  file_base=widget_base(tlb,row=4,/frame)
    infile_base=widget_base(file_base, row=2,xsize=xs)
      lbl1=widget_label(infile_base, value='Select Thermally Corrected file:',/align_left, xsize=xs)
...
; call the engine
;
m3_photom_full_correction_from_iof_engine, params=params
;
end
